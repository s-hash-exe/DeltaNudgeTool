...
export default class FreehandRoiTool extends BaseAnnotationTool {
  ...
  distanceFromPointCanvasForDeltaNudge(element, data, coords, imageDistance) {
    let distance = Infinity;

    if (!data) {
      return -1;
    }
    let secondCoords = { x: coords.x, y: coords.y + imageDistance };
    const canvasCoords = external.cornerstone.pixelToCanvas(element, coords);
    let updateHandleCanvas = external.cornerstone.pixelToCanvas(
      element,
      secondCoords
    );
    const distanceTosee = external.cornerstoneMath.point.distance(
      updateHandleCanvas,
      canvasCoords
    );
    const points = data.handles.points;
